...
namespace QuanLyQuanCafe.DAO
{
  public class MenuDAO
  {
    ...
    public List<Menu> GetListMenuByBillID(int id)
    {
      ...
      string query = "SELECT f.name, bi.count, f.price, f.price * bi.count AS totalPrice FROM dbo.BillInfo AS bi, dbo.Bill AS b, dbo.Food AS f WHERE bi.idBill = b.id AND bi.idFood = f.id AND b.status = 1 AND b.id = " + id;
